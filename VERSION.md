...
> 当前版本号以 `public/latest.json` 的 `version` 字段为准（设置页、版本检测、升级流程均读取该文件）。
> 本文件只记录各版本更新历史，不再承担"当前版本号"职责。发布新版时请同步更新 `public/latest.json`，并在下方「更新日志」追加本次记录。
...
**性能优化**
- 课堂管理列表接口瘦身，大幅降低加载时数据量
- 学生端课堂列表优先渲染，对话记录后台异步加载
- 学生端探究列表不再返回 htmlContent（HTML 懒加载）

**修复问题**
- 对话活动分析（pc）空模板/缺占位符时对话数据丢失，AI 误报未检测到对话数据
- 研究项目作业报告收集依赖 quizData，未选作业数据时报告收集为空
- 探究项目 subProjects 映射作用域错误导致 500
- HTTP 下 navigator.clipboard 不可用导致复制按钮报错（新增 copyText 降级工具）

**技术改进**
- 版本号统一以 public/latest.json 的 version 字段为准，VERSION.md 只记录更新历史
...
**功能增强**
- 学生项目提交面板：进入后不再显示聊天欢迎提示语
- 图片类作品点击预览支持 90° 旋转、左右翻转、放大缩小、拖拽查看与一键重置
